...
const Add = ({ token }) => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      // Add images to form data if available
  ...
  return (
    <form onSubmit={onSubmitHandler} className="flex flex-col w-full items-start gap-3">
        ...
        <div className="flex gap-3">
          {['S', 'M', 'L', 'XL', 'XXL'].map((size) => (
            <div key={size} onClick={() => setSizes((prev) =>
              ...
              <p className={`${sizes.includes(size) ? "bg-black text-white" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>{size}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <button type="submit" className="w-28 py-3 mt-4 bg-black text-white cursor-pointer" disabled={loading}>
        {loading ? 'Adding...' : 'Add'} {/* Show loading indicator */}
      </button>
    </form>
  );
};
