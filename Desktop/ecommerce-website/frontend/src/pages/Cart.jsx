      ...
      <div>
        {cartData.length > 0 ? (
          cartData.map((item, index) => {
            ...
            // If no product data is found, render nothing for this item
            if (!productData) {
              return (
            ...
            return (
              <div key={index} className="py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4">
                <div className="flex items-start gap-6">
                  <img className="w-16 sm:w-20" src={productData?.image?.[0] || assets.placeholderImage} alt={productData?.name || 'Product'} />
                  <div>
                ...
                <img onClick={() => updateQuantity(item.id, item.size, 0)} className="w-4 mr-4 sm:w-5 cursor-pointer" src={assets.bin_icon} alt="Remove" />
              </div>
            );
          })
        ) : (
